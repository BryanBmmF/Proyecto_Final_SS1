...
if ($tipo === $TIPO_PERSONAL) {
    $filtros = $filtros . " AND cuenta_involucrada='$correo' ";
}

$sql = "SELECT * FROM TRANSACCION_FINANCIERA WHERE ID IS NOT NULL $filtros;";
...
if ($result) {
    ...
    $mandar['mensaje'] = 'NO SE PUDIERON OBTENER LAS TRANSACCIONES FINANCIERAS A PETICION DEL USUARIO: $correo, Detalles: ' . mysqli_error($bd);
    $mandar['result'] = false;
    echo json_encode($mandar);
}
